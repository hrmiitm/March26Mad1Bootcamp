from get_app import app, db
from models import User
from werkzeug.security import generate_password_hash as gph

# Create 1 Admin
admin = User(username='admin', email='admin@admin.com', password=gph('a'), isAdmin=True)
...
for (n, e, p) in users:
    user = User(username=n, email=e, password=gph(p))
    db.session.add(user)
    db.session.commit()
...
for (n, e, p) in creators:
    user = User(username=n, email=e, password=gph(p), isCreator=True)
    db.session.add(user)
    db.session.commit()
