...
def get_songs_stats():
    all_songs = Song.query.all() # len([<song>, ...]) = 4
...
@admin_required()
def admin():
    user = get_current_user() # <user> : User.query.filter_by(id = session.get("id")).first()
...
def admin_delete_song():
    user = get_current_user()
    song_id = request.args.get('song_id') # request.args
    song = Song.query.filter_by(id=song_id).first() # object
    if not song:
        flash('Song not found', 'danger')
    else:
        song.playlists = []
